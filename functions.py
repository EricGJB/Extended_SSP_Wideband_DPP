...
def expand_indexes(max_angle_indexes,down_sample_factor,expand_factor,G_angle):
    ...
    for max_angle_index in max_angle_indexes:
        ...
        for i in range(1,int(down_sample_factor*expand_factor)):
...
def AM_no_TTD(early_stop, epsilon, initial_A, FDBs, num_sc, num_antenna_bs, num_stream, num_rf, num_max_iter):
    HBFs_list = []
    A_list = []
    object_list = []
    ...
    HBFs = np.zeros((num_sc, num_antenna_bs, num_stream)) + 1j * np.zeros((num_sc, num_antenna_bs, num_stream))
    for n in range(num_sc):
        Dm = np.linalg.pinv(A).dot(FDBs[n])
        HBF = A.dot(Dm)
        # normalization
        HBF = HBF / np.linalg.norm(HBF) * np.sqrt(num_stream)
        HBFs[n] = HBF
    ...
    for i in range(num_max_iter):
        ...
        for n in range(num_sc):
            Theta_n = FDBs[n].dot(np.linalg.pinv(FBBs[n]))
            term = term + np.linalg.norm(FBBs[n]) ** 2 * Theta_n
        A = np.exp(1j * np.angle(term))
    ...
    return np.array(A_list), np.array(HBFs_list), np.array(object_list), time_elapse, i


def MF(H,num_antenna_bs,num_antenna_ue,num_user,p,sigma_2):
    assert H.shape == (num_antenna_ue,num_antenna_bs,num_user)
    H = np.reshape(H,(num_antenna_bs,num_user))
    H = np.transpose(H)
    V = np.transpose(np.conjugate(H))
...
def ZF_equal_PA(H,num_antenna_bs,num_antenna_ue,num_stream,num_user,p):
    assert H.shape == (num_antenna_ue,num_antenna_bs,num_user)
    H = np.reshape(H,(num_antenna_bs,num_user))
    H = np.transpose(H)
    V = np.linalg.pinv(H)
...
def ZF_WF_PA(H,num_antenna_bs,num_antenna_ue,num_user,p,sigma_2):
    assert H.shape == (num_antenna_ue,num_antenna_bs,num_user)
    H = np.reshape(H,(num_antenna_bs,num_user))
    H = np.transpose(H)
    ...
    # subchannel_energies = np.linalg.norm(H,axis=-1)**2
    
    subchannel_energies = np.real(np.diag(H.dot(V)))**2
    
    # different from the single user case, we have to sort the eq_noise first, from smallest to largest 
    sorted_indexes = np.argsort(subchannel_energies)[::-1]
    
    subchannel_energies = subchannel_energies[sorted_indexes]

    eq_noise_0 = num_stream / (rou * subchannel_energies)
    eq_noise = num_stream / (rou * subchannel_energies)
    ...
    # recover the original index order of the pa_vector
    pa_vector_original_order = np.zeros(num_stream)
    for i in range(num_stream):
        pa_vector_original_order[i] = pa_vector[np.where(sorted_indexes==i)]
    
    V = V * np.sqrt(np.expand_dims(pa_vector_original_order, axis=0))
