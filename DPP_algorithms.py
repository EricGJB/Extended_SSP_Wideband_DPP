...
#num_clusters = int(args.Nc) 


num_stream = 4
num_rf = 4 #num_clusters
...
for i in range(num_samples):
    ...
    #     H_subcarrier_dict[n] = H_subcarrier_true
    ...
    #     temp = H_subcarrier_dict[n].dot(fully_digital_beamformer)
    #     temp = temp.dot(np.transpose(np.conjugate(temp)))
    #     rate_subcarrier = np.log2(np.linalg.det(np.eye(num_antenna_ue) + rou / num_stream * temp))
    #     rate_subcarrier_list.append(np.real(rate_subcarrier))
...
expand_factor = 2
mean_angle_list = np.zeros((num_samples, num_rf))
for i in range(num_samples):
    H = H_list_est[i] # (num_sc,num_antenna_ue,num_antenna_bs)
    H = np.transpose(H,(0,2,1))
    max_angle_indexes = stage_1(H[::resolution], A_list_ideal[::resolution,:,::down_sample_factor], num_sc_used, num_antenna_bs, num_antenna_ue, num_rf)
    max_angle_indexes_expanded = expand_indexes(max_angle_indexes,down_sample_factor,expand_factor,G_angle)
    max_angle_indexes = stage_2(H[::resolution], A_list_ideal[::resolution,:,max_angle_indexes_expanded], num_sc_used, num_antenna_bs, num_antenna_ue, num_rf, max_angle_indexes_expanded,G_angle)
    mean_angle_list[i] = np.arcsin(sin_value_sc0[np.array(max_angle_indexes)])
...
#%% E-SSP with MSBL instead of SOMP
...
#
#def update_mu_Sigma(inputs,num_measurements,N):
...
#    sigma = tf.cast(inputs[3],tf.complex64)
#
#    mu_real_list = []
#    mu_imag_list = []
#    diag_Sigma_real_list = []   
#    term0=[]
#    term2=tf.constant(0,dtype=tf.complex64)
#    for i in range(num_measurements):
#        Phi = Phi_list[:,i]
#        y = y_list[:, i]
#        y = tf.expand_dims(y,axis=-1)
#        Rx_PhiH = tf.multiply(alpha_list[:, :, i:i + 1], tf.transpose(Phi, (0, 2, 1), conjugate=True))
#        inv = tf.linalg.inv(tf.matmul(Phi, Rx_PhiH)+sigma* tf.eye(N, dtype=tf.complex64))
...
#        
#        Sigma = tf.linalg.diag(alpha_list[:, :, i]) - tf.matmul(z,tf.transpose(Rx_PhiH,(0,2,1),conjugate=True))
#        
##        Phi_delta = tf.matmul(Phi, tf.linalg.diag(alpha_list[:,:,i]))
##        V_temp= sigma*tf.eye(N,dtype=tf.complex64) + tf.matmul(Phi_delta, tf.transpose(Phi, (0, 2, 1), conjugate=True))
##        Sigma = tf.linalg.diag(alpha_list[:,:,i]) - tf.matmul(tf.transpose(Phi_delta, (0, 2, 1), conjugate=True), tf.matmul(tf.linalg.inv(V_temp),Phi_delta))
#    
#        term0.append(tf.matmul(Phi,mu))
#        temp1=tf.reduce_sum(tf.linalg.diag_part(tf.matmul(tf.matmul(Phi,Sigma),tf.transpose(Phi, (0, 2, 1), conjugate=True))),axis=-1)
#        term2=term2+temp1
#
#    term0 = tf.concat(term0,axis=-1)
#    term0 = tf.transpose(term0,(0,2,1))
#    resid=y_list-term0
#    updated_sigma = ((tf.math.real(tf.linalg.norm(resid,axis=(-1,-2))**2) + tf.math.real(term2) )+0.0001) / (num_measurements*N+0.0001)
#
...
#
#    return mu_real_list, mu_imag_list, diag_Sigma_real_list, updated_sigma
#
#def M_SBL_layer(N, G, num_measurements):
#    Phi_real_imag = Input(shape=(num_measurements, N, G, 2))
#    y_real_imag = Input(shape=(num_measurements, N, 2))
#    alpha_list = Input(shape=(G, num_measurements))
#    sigma = Input(shape=(1,))
#    # update mu and Sigma
#    mu_real, mu_imag, diag_Sigma_real, updated_sigma = Lambda(lambda x: update_mu_Sigma(x, num_measurements,N))(
#        [Phi_real_imag, y_real_imag, alpha_list, sigma])
#    # update alpha_list
...
#    mu_square = tf.tile(mu_square_average,(1,1,num_measurements))
#    alpha_list_updated = Lambda(lambda x: x[0] + x[1])([mu_square, diag_Sigma_real])
#    alpha_list_updated = (alpha_list_updated+0.0001)/(1+0.0001)
#    model = Model(inputs=[Phi_real_imag, y_real_imag, alpha_list, sigma], outputs=[alpha_list_updated, mu_real, mu_imag, mu_square_average, updated_sigma])
#    model.compile(loss='mse')
#    return model
#
#num_measurements = num_sc*num_stream
#
#FDBs_list_tmp = np.transpose(FDBs_list,(0,3,1,2))
#FDBs_list_tmp = np.reshape(FDBs_list_tmp,(num_samples,num_measurements,num_antenna_bs))
#FDBs_list_tmp = np.expand_dims(FDBs_list_tmp,axis=-1)
#y_real_imag_list = np.concatenate([np.real(FDBs_list_tmp),np.imag(FDBs_list_tmp)],axis=-1)
#
#Phi = np.expand_dims(A_list_feasible,axis=-1)
#Phi_real_imag = np.concatenate([np.real(Phi),np.imag(Phi)],axis=-1)
#Phi_real_imag = np.expand_dims(Phi_real_imag, axis=0)
#Phi_real_imag = np.expand_dims(Phi_real_imag, axis=0)
#Phi_real_imag_list = np.tile(Phi_real_imag,(1,num_stream,1,1,1,1))
#Phi_real_imag_list = np.reshape(Phi_real_imag_list,(1,num_measurements,num_antenna_bs,G_angle,2))
#
#M_SBL_single_layer = M_SBL_layer(num_antenna_bs,G_angle,num_measurements)
#
#
#performance_list_SSP_feasible_MSBL = []
#
...
#    print('Testing sample %d' % i)    
#    
#    alpha_list = np.ones((1, G_angle, num_measurements))  # initialization
#    num_layers = 50
#    sigma = np.ones((1,1))
#    for j in range(num_layers):
#        if j % 10 == 0:
#            print('M-SBL iteration %d' % j)
#        [alpha_list, mu_real, mu_imag, mu_square_average, sigma] = M_SBL_single_layer.predict([Phi_real_imag_list, y_real_imag_list[i:i+1], alpha_list,sigma],batch_size=1)
#
##    max_angle_indexes = np.argsort(mu_square_average[0,:,0])[-num_rf:]
#
#    max_angle_indexes = peak_finder(mu_square_average[0,:,0],num_rf)
#
#    
#    plt.plot(mu_square_average[0,:,0])
#
#    sum_rate = 0
#    # Performance evaluation
#    for n in range(num_sc):
...
#        sum_rate = sum_rate + np.real(rate_subcarrier)
#
#    performance_list_SSP_feasible_MSBL.append(sum_rate/num_sc)
#
#print('Performance of E-SSP with M-SBL: %.4f\n'%np.mean(performance_list_SSP_feasible_MSBL))
...
# for num_sc_used in [1,2,4,8,16,32,64,128]:
...
# if early_stop == False:
#     io.savemat('./results/%d_%d_e.mat'%(num_antenna_bs,num_antenna_ue),{'objective':mse_with_iter/num_samples})
#     # save object_lists 
#     np.save('./results/object_lists_e_ssp.npy',object_lists)
#     np.save('./results/HBFs_lists_e_ssp.npy',HBFs_lists)
...
#%% wideband phase-shifter-only methods 
# AM without TTD
#num_max_iter = 20
#epsilon = 1e-6
#early_stop = True
...
#    random_A = np.reshape(random_A, (num_antenna_bs, num_rf))
#    
#    A_list, HBFs_list, object_list_dll, time_elapse, iters = AM_no_TTD(early_stop, epsilon, random_A, FDBs_list[i], num_sc, num_antenna_bs, num_stream, num_rf, num_max_iter)
#    
#    object_lists.append(object_list_dll)
#    HBFs_lists.append(HBFs_list)
#    
...
#    performance_list_dll.append(sum_rate/num_sc) 
#
#print('Mean number of iterations of AM without TTD: %d'%(total_iters/num_samples))
#print('Performance of AM without TTD, with random initialization: %.4f\n' %np.mean(performance_list_dll))
...
#performance_list = []     
#for i in range(num_samples):
#    # beam broadening to construct the RF matrix
#    F_RF = np.zeros((num_antenna_bs,num_rf),dtype=np.complex64)
#    
#    # consider the case L>N_RF first
#    for l in range(num_rf):
#        normalized_AoA = np.sin(mean_angle_list[i,l])/2
#        max_squint = np.abs(normalized_AoA)*W/fc
#        max_Ms = np.sqrt(num_antenna_bs/max_squint)
#        Ms = np.floor(max_Ms)
#        while 1/Ms*np.floor(num_antenna_bs/Ms)<max_squint:
#            Ms = Ms-1
...
#        V = int(V)
#        # print(V)
#        # 这种启发式设计不是最优的，如当V=1的时候，可画图观察
#        normalized_AoA_subarray_list = normalized_AoA + np.sign(normalized_AoA)*(2*(np.arange(V)-V//2+1)-1)/(2*Ms) 
#        fai_subarray_list = np.pi/Ms*np.arange(V)
#        analog_beamformer_broadened = np.zeros(num_antenna_bs,dtype=np.complex64)
#        for v in range(V):
#            for m in range(Ms):
#                analog_beamformer_broadened[v*Ms+m] = np.exp(-1j*2*np.pi*(v*Ms+m)*normalized_AoA_subarray_list[v])\
#                                                        *np.exp(-1j*fai_subarray_list[v])/np.sqrt(V*Ms)
#        F_RF[:,l] = analog_beamformer_broadened
#        
#    A = F_RF
#    sum_rate = 0
#    # Performance evaluation
...
#        sum_rate = sum_rate + np.real(rate_subcarrier)
#
#    performance_list.append(sum_rate/num_sc) 
#    
#print('Beam broadening performance: %.3f'%np.mean(performance_list))
